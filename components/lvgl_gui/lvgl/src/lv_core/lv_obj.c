 ...
 * @param obj pointer to an object to del
...
lv_res_t lv_obj_del(lv_obj_t * obj)
{
    ...
    /*Send a signal to the parent to notify it about the child del*/
    if(par) {
...
#if LV_USE_ANIMATION
/**
 * A function to be easily used in animation ready callback to del an object when the animation is ready
 ...
 * Useful for cases where you can't del an object directly in an `LV_EVENT_DELETE` handler (i.e. parent).
 * @param obj object to del
 * @see lv_async_call
 */
...
static void obj_del_core(lv_obj_t * obj)
{
    ...
    /* Reset all input devices if the object to del is used*/
 ...
 * @param tr_limit del transitions only "older" then this. `NULL` is not used
 */
static void trans_del(lv_obj_t * obj, uint8_t part, lv_style_property_t prop, lv_style_trans_t * tr_limit)
{
    lv_style_trans_t * tr;
...
static void trans_anim_start_cb(lv_anim_t * a)
{
    ...
    /*Init prop to an invalid values to be sure `trans_del` won't del this added `tr`*/
    tr->prop = 0;
