...
void lv_anim_start(lv_anim_t * a)
{
    ...
    if(a->exec_cb != NULL) lv_anim_del(a->var, a->exec_cb); /*fp == NULL would del all animations of var*/
 ...
 *           or NULL to del all the animations of 'var'
...
bool lv_anim_del(void * var, lv_anim_exec_xcb_t exec_cb)
{
    ...
    while(a != NULL) {
        ...
        if(a->var == var && (a->exec_cb == exec_cb || exec_cb == NULL)) {
            ...
            lv_mem_free(a);
            anim_mark_list_change(); /*Read by `anim_task`. It need to know if a del occurred in
                                         the linked list*/
            del = true;
        }
...
static void anim_task(lv_task_t * param)
{
    ...
    while(a != NULL) {
        /*It can be set by `lv_anim_del()` typically in `end_cb`. If set then an animation del
        ...
        /* If the linked list changed due to anim. del then it's not safe to continue
 ...
 * e.g. repeat, play back, del etc.
 * @param a pointer to an animation descriptor
 * @return true: animation del occurred and the `LV_GC_ROOT(_lv_anim_ll)` has changed
 * */
static bool anim_ready_handler(lv_anim_t * a)
{
    ...
    if(a->repeat_cnt == 0 && ((a->playback_time == 0) || (a->playback_time && a->playback_now == 1))) {
        ...
        /*Create copy from the animation and del the animation from the list.
