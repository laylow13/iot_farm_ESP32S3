...
/**
 * Automatically del the message box after a given time
 * @param mbox pointer to a message box object
 * @param delay a time (in milliseconds) to wait before del the message box
 */
void lv_msgbox_start_auto_close(lv_obj_t * mbox, uint16_t delay)
{
    ...
    if(lv_msgbox_get_anim_time(mbox) != 0) {
        ...
        /*Create an animation to del the mbox `delay` ms later*/
        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, mbox);
